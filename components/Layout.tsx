...
import { Bus, LogOut, Menu, User as UserIcon, X } from 'lucide-react';
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 font-sans">
      <nav className="bg-gradient-to-r from-brand-start to-brand-end shadow-lg sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex space-x-8 items-center">
              {user?.role === UserRole.CLIENT && (
                <>
                  <Link to="/" className={isActive('/')}>Rechercher</Link>
                  <Link to="/my-reservations" className={isActive('/my-reservations')}>Mes Réservations</Link>
                </>
              )}
              ...
              {user ? (
                ...
                <div className="flex space-x-4">
                  ...
                  <Link to="/signup" className="bg-white text-indigo-600 hover:bg-indigo-50 px-4 py-2 rounded-full font-medium shadow-sm transition-all transform hover:scale-105">
                    Inscription
                  </Link>
                </div>
              )}
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-indigo-800 px-4 pt-2 pb-4 space-y-2">
             {user ? (
                <>
                  {user.role === UserRole.CLIENT && <Link to="/" className="block text-white py-2">Rechercher</Link>}
                  {user.role === UserRole.CLIENT && <Link to="/my-reservations" className="block text-white py-2">Mes Réservations</Link>}
               ...
               <>
                ...
                <Link to="/signup" className="block text-white py-2">Inscription</Link>
               </>
             )}
          </div>
        )}
